...
import CashAllCaseStudy from './pages/CashAllCaseStudy';
  ...
  const renderSection = () => {
    switch (activeTab) {
      case 'home':
        return (
          <Home 
            ...
            onWhatIDo={() => setActiveTab('what-i-do')}
          />
        );
      case 'about':
        return <About />;
      case 'what-i-do':
        return (
          <WhatIDo 
            onExploreWork={() => setActiveTab('projects')} 
            onContact={() => setActiveTab('contact')} 
          />
        );
      case 'projects':
        return (
          <Projects 
            onViewCaseStudy={(route) => setActiveTab(route)} 
            onContact={() => setActiveTab('contact')} 
          />
        );
      case 'content':
      ...
      case 'case-study-cashall':
        return <CashAllCaseStudy onBack={() => setActiveTab('projects')} />;
  ...
  return (
    <div className="min-h-screen bg-bg-dark text-text-main selection:bg-primary/30 selection:text-white relative overflow-hidden transition-colors duration-1000">
      ...
      <main className={cn(
        ...
        "pt-28 md:pt-36",
        "pb-24 px-4 md:px-10 max-w-7xl mx-auto overflow-visible"
      )}>
